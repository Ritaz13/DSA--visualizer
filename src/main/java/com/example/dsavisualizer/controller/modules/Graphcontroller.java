...
public class Graphcontroller extends ModuleController {
    ...
    private static final long DEFAULT_SPEED_MS = 1500;
    ...
    private Map<Integer, Integer> finishTime = new HashMap<>();
    private int time = 0;

    private Map<Integer, Color> pathColors = new HashMap<>();
    private Map<Integer, String> nodeStatus = new HashMap<>();

    private List<String> dsStates;
    private List<Set<Integer>> visitedStates;
    ...
    private void performDFSTraversal(int start) {
        ...
        stack.push(start);
        nodeStatus.put(start, "gray");
        pathColors.put(start, Color.GRAY);
        discoveryTime.put(start, time++);
        ...
        while (!stack.isEmpty()) {
            ...
            for (int neighbor : graphData.getNeighbors(node)) {
                String neighStatus = nodeStatus.get(neighbor);
                if (neighStatus.equals("white")) {
                    for (Edge edge : graphData.getEdges()) {
                    ...
                    nodeStatus.put(neighbor, "gray");
                    pathColors.put(neighbor, Color.GRAY);
                    ...
                    dsStates.add("Stack: " + stack.toString());
                    break;
                } else if (neighStatus.equals("gray")) {
                    for (Edge edge : graphData.getEdges()) {
                        if (edge.from == node && edge.to == neighbor) {
                            edge.edgeType = "back";
                ...
                } else if (neighStatus.equals("green")) {
                    if (discoveryTime.get(neighbor) > discoveryTime.get(node)) {
                        for (Edge edge : graphData.getEdges()) {
                            if (edge.from == node && edge.to == neighbor) {
            ...
            if (!hasUnvisitedNeighbor) {
                ...
                nodeStatus.put(finishedNode, "green");
                pathColors.put(finishedNode, Color.GREEN);
                finishTime.put(finishedNode, time++);
                visitedNodes.add(finishedNode);
                visitedStates.add(new HashSet<>(visitedNodes));
                dsStates.add("Stack: " + stack.toString());
            }
        }
    ...
    private void performBFSTraversal(int start) {
        ...
        while (!queue.isEmpty()) {
            ...
            pathColors.put(node, Color.GREEN);

            visitedStates.add(new HashSet<>(visitedNodes));
            dsStates.add("Queue: " + queue.toString());

            for (int neighbor : graphData.getNeighbors(node)) {
                if (nodeStatus.get(neighbor).equals("white")) {
                    for (Edge edge : graphData.getEdges()) {
                        if (edge.from == node && edge.to == neighbor) {
                            edge.edgeType = "tree";
                            edge.color = Color.GREEN;
...
=======
>>>>>>> f0ccd5234876f8ea4ec7c32ef530524620df59e2
    ...
    private void redrawCanvas() {
        ...
        for (Integer node : graphData.getNodes()) {
            ...
            Color color = pathColors.getOrDefault(node, Color.WHITE);
            double radius = 20;
            gc.setFill(color);
            gc.fillOval(pos[0] - radius, pos[1] - radius, 2 * radius, 2 * radius);
            gc.setStroke(Color.BLACK);
            gc.setLineWidth(2);
            gc.strokeOval(pos[0] - radius, pos[1] - radius, 2 * radius, 2 * radius);

            gc.setFill(Color.BLACK);
            gc.setFont(new Font(14));
            gc.fillText(String.valueOf(node), pos[0] - 6, pos[1] + 5);
        }

        if (currentTraversalIndex >= 0 && currentTraversalIndex < dsStates.size()) {
            String dsState = dsStates.get(currentTraversalIndex);
            drawDataStructureBoxes(gc, dsState, canvas.getWidth(), canvas.getHeight());
        }
    }
...
>>>>>>> f0ccd5234876f8ea4ec7c32ef530524620df59e2
    ...
    private void drawDataStructureBoxes(GraphicsContext gc, String dsState, double canvasWidth, double canvasHeight) {
        String[] parts = dsState.split(": ");
        if (parts.length < 2) return;
        String type = parts[0];
        String listStr = parts[1].replace("[", "").replace("]", "").replace(" ", "");
        String[] elements = listStr.isEmpty() ? new String[0] : listStr.split(",");

        double boxSize = 30;
        double spacing = 5;
        double startX = canvasWidth - (elements.length * (boxSize + spacing)) - 20;
        double startY = canvasHeight - boxSize - 20;

        gc.setFont(new Font(14));
        gc.setFill(Color.BLACK);
        gc.fillText(type + ":", startX - 50, startY + 20);

        if (type.equals("Queue")) {
            double y = startY;
            for (int i = 0; i < elements.length; i++) {
                double x = startX;
                Color fillColor = (i == 0) ? Color.YELLOW : Color.LIGHTGRAY;
                gc.setFill(fillColor);
                gc.fillRect(x, y, boxSize, boxSize);
                gc.setStroke(Color.BLACK);
                gc.strokeRect(x, y, boxSize, boxSize);
                gc.setFill(Color.BLACK);
                gc.fillText(elements[i], x + boxSize / 2 - 5, y + boxSize / 2 + 5);
                y -= (boxSize + spacing);
            }
        } else {
            for (int i = 0; i < elements.length; i++) {
